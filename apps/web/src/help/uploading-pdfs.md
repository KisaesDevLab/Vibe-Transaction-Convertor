...
- **Text-layer or scanned.** The pipeline auto-detects. Scanned PDFs are routed through GLM-OCR; text-layer PDFs skip OCR entirely.
...
- **"GLM_OCR_URL is not set"** — the PDF is scanned and the OCR engine isn't configured. Either configure it on `/admin/engines` or upload a text-layer PDF instead.
