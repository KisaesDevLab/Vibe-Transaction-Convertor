...
- **OCR runs locally on GLM-OCR; extraction on local Ollama (ADR-025, amends ADR-023).** Scanned/image statements are rasterized and transcribed to markdown by a locally-hosted **GLM-OCR** llama-server (OpenAI-compatible `/v1/chat/completions` vision), then the local **Ollama** text model (`qwen2.5:32b-instruct`) extracts the schema JSON from that markdown (two-stage). Text-layer statements skip OCR and go straight to the text model. Check payees are read via GLM-OCR transcribe→text-parse, falling back to the local Ollama vision model **`qwen3-vl:30b`**. MiniCPM-V is removed from the OCR path. OCR output is **cleartext** markdown (no `<ENTITY_N>` tokens, no materialize step).
...
- **OCR:** Local **GLM-OCR** llama-server (OpenAI-compatible `/v1/chat/completions` vision) transcribes scanned pages to markdown; on-appliance, zero egress. Check-payee fallback uses local Ollama **`qwen3-vl:30b`**. (Replaces MiniCPM-V for OCR; ADR-025, amends ADR-023.)
